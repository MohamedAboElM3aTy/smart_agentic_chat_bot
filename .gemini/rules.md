...
<!-- You will build a nice movies app that will populate the top rated movies and the user can show the movie details. -->
You will build a simple ecommerce app consists of:
1- Signup and login flow connected with firebase 
2- the registered users will be added to tables in firebase 
3- We will build a small database in firestore 
4- The main functionality here is to fetch those products from firestore (we will split them to categories) and arrange them 
5- The second core is to build a chatbot allowing us to ask like 
-- Give me the products less than 50 EGP
-- The chatbot will search on the firestore database to fetch and list the products to the user. 
...
- **Clarification:** If a request is ambiguous, ask for clarification on the
  ...
  server).
  **\*Networking:** Don't forget to add in the interceptor the Accept app/json and to also include the tokeation jsn.
...
- **Composition over Inheritance:** Favor composition for building complex
  widgets and logic.
  - **Structure:** preferred to use DIO package for api connection.
- **Immutability:** Prefer immutable data structures. Widgets (especially
  `StatelessWidget`) should be immutable.
- **State Management:** Separate ephemeral state and app state. Use a state
  management solution for app state to handle the separation of concerns, if you will use a state managmenet prefer to use a riverpod.
...
- **Architecture:** Use the clean architecture we will split them into data, domains and presentation that hold the widget pages and state management.
